            ...
            when (event!!.sensor.type) {
                ...
                Sensor.TYPE_LINEAR_ACCELERATION -> {
                    ...
                    if (heroPDR.isStep(event.values.clone(), accMatrix)) {
                        ...
                        coorPdr = arrayListOf(
                            coorPdr[0] + (sin(Math.toRadians(pdrAngle.toDouble())) * stepLength*10),
                            coorPdr[1] + (cos(Math.toRadians(pdrAngle.toDouble())) * stepLength*10),
                            coorPdr[2])
                        rfLocalization.coorPdr = coorPdr

                        if(199.0 < rfStatusCode && rfStatusCode < 300.0){
                            coor = rfResult["rfCoor"] as ArrayList<Double>
                        }
                    }
                }
            }
